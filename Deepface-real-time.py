...
#detector_path = path+"haarcascade_frontalface_default.xml"
...
acquaintance_pictures = "database"

acquaintances = dict()

for file in listdir(acquaintance_pictures):
	acquaintance, extension = file.split(".")
	img_path = 'database\%s.jpg' % (acquaintance)
	img = detectFace(img_path)
	
	representation = model.predict(img)[0]
	
	acquaintances[acquaintance] = representation
	
print("acquaintance representations retrieved successfully")
	...
	for (x,y,w,h) in faces:
		if w > 130: #discard small detected faces
			...
			for i in acquaintances:
				acquaintance_name = i
				source_representation = acquaintances[i]
			...
			for i in acquaintances:
				...
				if index == np.argmin(distances):
					if distances[index] <= 0.70:
						...
						print("detected: ",acquaintance_name, "(",distances[index],")")
						acquaintance_name = acquaintance_name.replace("_", "")
			...
			if is_found:
				display_img = cv2.imread("database\%s.jpg" % acquaintance_name)
				pivot_img_size = 112
				...
				try:
					...
					label = acquaintance_name+" ("+"{0:.2f}".format(similarity)+")"
