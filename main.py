...
def evaluate_model(model, dataset, batch_size, frame_count, shuffle, save_eval_path):
    ...
    print(
        classification_report(
            all_labels, all_preds, target_names=dataset.classes, zero_division=0
        )
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Abnormal Behavior and Stranger Detection"
    )
    parser.add_argument(
        "--root_dir", type=str, required=True, help="Path to dataset root folder"
    )
    parser.add_argument(
        "--frame_count",
        type=int,
    ...
    parser.add_argument(
        "--save_eval_path", type=str, default=None, help="Path to save CSV report"
    )
...
def main():
    ...
    dataset = BehaviorModelDataset(root_dir=args.root_dir, frame_count=args.frame_count)
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=args.shuffle)
    num_classes = len(dataset.classes)
    model = BehaviorModel(feature_dim=1280, hidden_size=512, num_classes=num_classes)
    ...
    if args.mode == "train":
        ...
        evaluate_model(
            ...
            save_eval_path=args.save_eval_path,
        )
